...
public class PonyData implements IPonyData {
    ...
    public static Memoize<IPonyData> parse(@Nullable Identifier identifier) {
        ...
        return MinecraftClient.getInstance().getResourceManager().getResource(identifier).flatMap(res -> {
            try {
                ...
                MineLittlePonyClient.logger.warn("Unable to read {} metadata", identifier, e);
            }
            return null;
            ...
            return Memoize.load(callback -> {
                NativeUtil.parseImage(identifier, img -> {
                    ...
                    MineLittlePonyClient.logger.fatal("Unable to read {} metadata", identifier, e);
    ...
    public Sizes getSize() {
        Sizes sz = MineLittlePonyClient.getInstance().getConfig().sizeOverride.get();

        if (sz != Sizes.UNSET) {
            return sz;
        }

        if (size == Sizes.UNSET || !MineLittlePonyClient.getInstance().getConfig().sizes.get()) {
            return Sizes.NORMAL;
        }
