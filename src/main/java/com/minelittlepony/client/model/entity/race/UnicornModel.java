...
public class UnicornModel<T extends LivingEntity> extends EarthPonyModel<T> implements IUnicorn {
    ...
    public boolean isCasting() {
        return MineLittlePonyClient.getInstance().getConfig().tpsmagic.get()
    ...
    public ModelPart getArm(Arm side) {
        if (hasMagic() && getArmPoseForSide(side) != ArmPose.EMPTY && MineLittlePonyClient.getInstance().getConfig().tpsmagic.get()) {
    ...
    protected void positionheldItem(Arm arm, MatrixStack matrices) {
        ...
        if (!MineLittlePonyClient.getInstance().getConfig().tpsmagic.get() || !hasMagic()) {
            return;
        }
